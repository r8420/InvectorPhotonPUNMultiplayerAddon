    ...
    private void PUN_ConvertvItemCollection(GameObject obj) {
        if (obj.GetComponent<vItemCollection>()) {
            ...
            if (!obj.GetComponent<PUN_ItemCollect>()) {
                ...
                for (int i = 0; i < obj.GetComponent<vItemCollection>().onPressActionInputWithTarget.GetPersistentEventCount(); i++) {
                    if (obj.GetComponent<vItemCollection>().onPressActionInputWithTarget.GetPersistentMethodName(i) == "NetworkDestroy") {
                        UnityEventTools.RemovePersistentListener(obj.GetComponent<vItemCollection>().onPressActionInputWithTarget, i);
                    }
                }
                obj.GetComponent<vItemCollection>().OnPressActionInput.AddListener(obj.GetComponent<PUN_ItemCollect>().NetworkDestory);
                UnityEventTools.AddPersistentListener(obj.GetComponent<vItemCollection>().OnPressActionInput, obj.GetComponent<PUN_ItemCollect>().NetworkDestory);
            }
